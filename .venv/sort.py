...
for nowpage in range(total_pages):
    ...
    for user in next_data:
        student_user = user.get('user', {}).get('studentUser', {})
        student_name = student_user.get('name')
        if student_name:
            name_data.append(student_name)
...
input_names = input("请输入名字:").split()
name_index = {name: index for index, name in enumerate(name_data)}
sorted_names = sorted(input_names, key=lambda name: name_index.get(name, float('inf')))
